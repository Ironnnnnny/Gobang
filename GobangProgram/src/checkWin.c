...
/*void check_row(int **state,int row,int column, int flag)
{
    //判断行是否满足条件
    if((column - 4) >=0)
        begin0 = (column - 4);
...
void check_column(int **state,int row,int column, int flag)
{
    //判断列是否满足条件
    if((row - 4) >= 0 )
        begin0 = (row - 4) ;
...
void check_diagonal(int **state,int row,int column, int flag)
{
...
void check_dis_diagonal(int **state,int row,int column, int flag)
{
    //判断副对角线是否满足条件
    if( (row - 1) >(N - column) )
        len = (N - column) ;
